(function(){
  ...
  if(!form)return;
  const flutterTheme=document.body.classList.contains("customer-ui-theme-flutter");
  ...
  async function refreshPickerAvailability(ctx){
    ...
    visible.forEach((card,index)=>{
      ...
      if(data.available){
        ...
        if(action){action.disabled=false;action.textContent=pickerIndex===null?(flutterTheme?"اختيار هذا الملعب":"إضافة هذا الملعب"):(flutterTheme?"استبدال هذا الملعب":"استبدال الملعب");action.className="booking-picker-resource-action";}
      }else{
        if(status){status.className="booking-picker-resource-status busy";status.textContent=data.reason||"غير متاح في هذا الوقت";}
        if(action){action.disabled=false;action.textContent=flutterTheme?"إظهار أوقات بديلة":"تحديث الأوقات";action.className="booking-picker-resource-action busy-action";}
  ...
  durationButtons.forEach(button=>{
    button.addEventListener("click",()=>{
      ...
      if(start){
        slotStatus.textContent="تم تحديث المدة إلى "+durationValue+" دقيقة. "+(flutterTheme?"اضغط «اختيار ملعب»":"اضغط «إضافة ملعب»")+" لاختيار الملاعب.";
  ...
  timeInput?.addEventListener("input",()=>{
    ...
    availabilityDebounce=setTimeout(()=>{
      slotStatus.textContent="تم تحديد "+value+" — "+(flutterTheme?"اضغط «اختيار ملعب»":"اضغط «إضافة ملعب»")+" لفحص الملاعب لهذا الموعد.";
      slotStatus.className="booking-slot-status";
      if(picker&&!picker.hidden)refreshPickerAvailability(globalContext());
    },140);
  });
