...
def ui():
    return render_template(
        ...
        categories=NewsCategory.query.filter_by(is_active=True).all(),
    )
...
def create():
    ...
    try:
        ...
        if not title:
            raise ValueError("عنوان الخبر مطلوب")

        cover_url = save_uploaded_media(request.files.get("cover_image"), "news", "image")
        ...
        db.session.rollback()
        return render_template("news/form.html", categories=categories, error=str(exc)), 400
...
def category_create():
    ...
    if not name:
        return {"error": "اسم التصنيف مطلوب"}, 400
    if NewsCategory.query.filter_by(name_ar=name).first():
        return {"error": "التصنيف موجود"}, 400
    db.session.add(NewsCategory(name_ar=name))
    db.session.commit()
    return redirect(url_for("news.ui"))
...
def api():
    return jsonify(
        [
            {"id": r.id, "title_ar": r.title_ar, "status": r.status}
            for r in Post.query.order_by(Post.id.desc()).limit(50).all()
        ]
    )
