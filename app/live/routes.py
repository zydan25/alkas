...
@login_required
def ui():
    events = LiveEvent.query.order_by(LiveEvent.starts_at.desc(), LiveEvent.id.desc()).limit(60).all()
    streams = Stream.query.order_by(Stream.id.desc()).limit(60).all()
    return render_template("live/index.html", events=events, streams=streams)
