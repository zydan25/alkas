...
@login_required
def ui():
    return render_template("ads/index.html",campaigns=AdCampaign.query.order_by(AdCampaign.id.desc()).limit(70).all(),creatives=AdCreative.query.order_by(AdCreative.id.desc()).limit(70).all())
@bp.get("/api")
@login_required
def api():
